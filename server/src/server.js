...
function postStatusUpdate(user, location, contents) { // If we were implementing this for real on an actual server, we would check
    // that the user ID is correct & matches the authenticated user. But since
    // we're mocking it, we can be less strict.
    // Get the current UNIX time.
    var time = new Date().getTime(); // The new status update. The database will assign the ID for us.
    var newStatusUpdate = {
        "likeCounter": [],
        "type": "statusUpdate",
        "contents": {
        ...
        "comments": []
    }; // Add the status update to the database.// Returns the status update w/ an ID assigned.
    newStatusUpdate = addDocument('feedItems', newStatusUpdate); // Add the status update reference to the front of the current user's feed.
...
app.post('/feeditem', validate({
    ...
    var body = req.body;
    var fromUser = getUserIdFromToken(req.get('Authorization')); // Check if requester is authorized to post this status update.
    // (The requester must be the author of the update.)
    if (fromUser === body.userId) {
        var newUpdate = postStatusUpdate(body.userId, body.location, body.contents); // When POST creates a new resource, we should tell the client about it
        // in the 'Location' header and use status code 201.
        res.status(201);
        res.set('Location', '/feeditem/' + newUpdate._id); // Send the update!
        res.send(newUpdate);
...
app.put('/feeditem/:feeditemid/content', function(req, res) {
  ...
  app.delete('/feeditem/:feeditemid', function(req, res) {
      ...
      if (feedItem.contents.author === fromUser) {
          ...
          feedIds.forEach((feedId) => {
              ...
              if (itemIdx !== -1) {
                  // Splice out of array.
                  feed.contents.splice(itemIdx, 1); // Update feed.
  ...
  //likeFeedItemWe decided to map this
  //function toPUT / feeditem / feeditemid / likelist / userid.Implement this route inserver / src / server.js:
  // Like a feeditem.
  ...
  app.delete('/feeditem/:feeditemid/likelist/:userid', function(req, res) {
      ...
      if (fromUser === userId) {
          ...
          // Remove from likeCounter if present49
          if (likeIndex !== -1) {
          ...
          // Return a resolved version of the likeCounter// Note that this request succeeds even if the
          // user already unliked the request!
          res.send(feedItem.likeCounter.map((userId) => database.readDocument('users', userId)));
      } else {
          // 401: Unauthorized.
          res.status(401).end();
      }
  ...
  app.post('/search', function(req, res) {
      ...
      if (typeof(req.body) === 'string') {
          ...
          // arrays. It takes an anonymous function, which it calls// with each item in the array. If that function returns 'true',
          // it will include the item in a return array. Otherwise, it will// not.// Here, we use filter to return only feedItems that contain the
          // query text.// Since the array contains feed item IDs, we later map the filtered
...
app.post('/resetdb',function(req, res) {
  console.log("Resetting database...");// This is a debug route, so don't do any validation.
database.resetDatabase();// res.send() sends an empty response with status code 200
res.send();});
...
function getFeedItemSync(feedItemId) {
  ...
  feedItem.likeCounter = feedItem.likeCounter.map((id) => database.readDocument('users', id));
  // Assuming a StatusUpdate. If we had other types of FeedItems in the DB, we would
  // need to check the type and have logic for each type.
  feedItem.contents.author = database.readDocument('users', feedItem.contents.author);
...
function getUserIdFromToken(authorizationLine) {
    try { // Cut off "Bearer " from the header value.
